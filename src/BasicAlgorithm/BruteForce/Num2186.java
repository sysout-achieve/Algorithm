...
public class Num2186 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i <= row; i++) {
            for (int j = 1; j <= col; j++) {
                if (map[i][j] == firstChar) answer += dfs(i, j, 0, target);
            }
        }
//        bfs(target, size);       결과 나오지만 메모리 초과
    ...
    static int dfs(int x, int y, int index, String target) {
        int result = 0;
        if (index == size) {
            return 1;
        }
        if (dp[(x * (col - 1)) + y][index] == -1){
            return 0;
        }
        ...
        for (int i = 1; i <= moveCnt; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                if (dx > 0 && dy > 0 && dx <= row && dy <= col && c == map[dx][dy]) {
                    result += dfs(dx, dy, index + 1, target);
                    isAble = true;
                }
            }
        }
